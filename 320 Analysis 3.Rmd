...
Since this is America, we want to drop that Celsius nonsense like it is hot. I want you to create a function called `cels.2.fahr()` that converts a temperature (or vector of temperatures) measured in Celsius to the equivalent temperature(s) in Fahrenheit. 

Code and Output:
```{r}
cels.2.fahr = function(data){
  data = data*1.8 + 32
  return(data)
}
...
for(j in 1:12){    #DO NOT CHANGE
  for(k in 1:7){   #DO NOT CHANGE
    ...
    MEAN.TEMP.TRAIN[j,k] = mean(filter(TRAIN.FINAL, Month == months[j], Day == days[k])$Temperature)
  }                #DO NOT CHANGE
}                  #DO NOT CHANGE
...
# Adds a new value to an established vector
```
...
for(i in 1:1461){   #DO NOT CHANGE
  if (TRAIN.FINAL$Day[i] == "Saturday" || TRAIN.FINAL$Day[i] == "Sunday"){
    WEEKEND.HUMIDITY = c(WEEKEND.HUMIDITY, TRAIN.FINAL$Humidity[i])
  }else{
    WEEKDAY.HUMIDITY = c(WEEKDAY.HUMIDITY, TRAIN.FINAL$Humidity[i])
  }
}                   #DO NOT CHANGE
...
Answer (4 Points): Essentially, we compared if the mean of the humidity on weekends was different than the mean of the humidity on weekdays. Our test gave us a certain probability of the difference occurring by chance, and if this value was small enough, we could say confidently that the means were different. However, our probability value was too high, so we can't conclude that the mean humidity is different. 
...
for (i in 2:1461){
  TRAIN.FINAL$`Pressure Percent Change`[i] = (TRAIN.FINAL$Pressure[i] -  TRAIN.FINAL$Pressure[i-1])/(TRAIN.FINAL$Pressure[i-1])
}
...
for (i in 2:1460){
  TRAIN.FINAL$`Pressure 3-Day Average`[i] = (TRAIN.FINAL$Pressure[i] +  TRAIN.FINAL$Pressure[i-1] + TRAIN.FINAL$Pressure[i + 1])/3
}
